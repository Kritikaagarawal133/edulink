...
const registerProfile = async (profile) => {
  ...
  const message = `Thank you for registering at SecuriCash. Have a great day ahead. 
  Please confirm your email using the given link to continue to the site. ${CLIENT_URL}/confirm?userId=${user._id}`;
  await sendMail(email, 'Thank you for registering at SecuriCash', message);
...
const login = async (req, res, next) => {
  try {
    ...
    if (user.password === req.body.password) {
      const accessToken = jwt.generateJwtToken(user);
